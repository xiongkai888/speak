...
import com.data.volley.DefaultRetryPolicy;
import com.data.volley.NetworkResponse;
import com.data.volley.Request;
import com.data.volley.Response;
import com.data.volley.Response.ErrorListener;
...
public abstract class MultiPartRequest<T> extends Request<T> implements ProgressListener{
	...
	private Map<String, List<String>> mFileUploads = null;
	...
	public MultiPartRequest(int method, String url, Listener<T> listener, ErrorListener errorListener) {
		...
		mFileUploads = new HashMap<String, List<String>>();
	...
	public MultiPartRequest<T> addFile(String name, String filePath) {
		List<String> listFilePaths = mFileUploads.get(name);
		if (listFilePaths == null) {
			listFilePaths = new ArrayList<String>();
			mFileUploads.put(name, listFilePaths);
		}
...
//		mFileUploads.put(name, filePath);
	...
	public boolean hasFileToUpload() {
		return mFileUploads != null && !mFileUploads.isEmpty();
	}
