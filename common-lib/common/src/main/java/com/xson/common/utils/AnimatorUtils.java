...
public class AnimatorUtils {
    private static AnimatorUtils sInstance = new AnimatorUtils();

    public static  AnimatorUtils getInstance() {
        return sInstance;
    }

    private AnimatorRunner mRunner;

    /** Private constructor for singleton */
    private AnimatorUtils() {
        if(Build.VERSION.SDK_INT >= 14) {
            mRunner = new AnimatorRunner();
        }
    }

    @TargetApi(Build.VERSION_CODES.ICE_CREAM_SANDWICH)
    public void removeView(final View victim) {
        if(Build.VERSION.SDK_INT < 14) {
    ...
    public void slideAndFadeIn(final ViewGroup target, final int previousHeight) {
        final int visibility = target.getVisibility();
        if(Build.VERSION.SDK_INT < 14) {
            target.setVisibility(visibility == View.GONE ? View.VISIBLE : View.GONE);
            return;
        }
        mRunner.endOldAnimation();
    ...
    public static void placeFocusAtTopOfScreenAfterReLayout(final View view) {
        ...
        SchedulingUtils.doAfterLayout(view, new Runnable() {
            @Override
            public void run() {
                getParentScrollView(view).clearFocus();
            }
        });
    }
