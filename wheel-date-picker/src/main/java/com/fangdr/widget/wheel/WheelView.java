...
import com.fangdr.widget.wheel.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	...
	private int[] changedItems = new int[] {0, -1};
		...
		if (index != currentItem) {
			if (animated) {
				...
				if(changedItems == null)
					changedItems = new int[2];
				changedItems[0] = currentItem;
				changedItems[1] = old;
	...
	private void initResourcesIfNecessary() {
		...
		if (topShadow == null && SHADOWS_COLORS != null) {
			topShadow = new GradientDrawable(Orientation.TOP_BOTTOM, SHADOWS_COLORS);
		}

		if (bottomShadow == null && SHADOWS_COLORS != null) {
	...
	private boolean rebuildItems() {
		...
		if(changedItems != null) {
			...
			int oldIndex = changedItems[1] - firstItem;
			int newIndex = changedItems[0] - firstItem;
			View oldView = oldIndex < 0 || oldIndex >= count ? null : itemsLayout.getChildAt(oldIndex);
			View newView = newIndex < 0 || newIndex >= count ? null : itemsLayout.getChildAt(newIndex);
			viewAdapter.onCenterItemChanged(changedItems[1], oldView, changedItems[0], newView);
		}
